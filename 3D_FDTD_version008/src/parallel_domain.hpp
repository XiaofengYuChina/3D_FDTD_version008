...
// Compatible with: Non-uniform meshes, two-level system gain medium, CPML
...
struct ParallelConfig {
    ...
    void validate(size_t NxT, size_t NyT, size_t NzT) {
        ...
        if (num_domains > (int)max_domains) {
            std::cout << "[Parallel] Reducing domains from " << num_domains
                      << " to " << max_domains << "\n";
...
public:
    ...
    void initialize(const ParallelConfig& cfg,
        ...
        if (!config.enabled) {
            std::cout << "[Parallel] Domain decomposition disabled\n";
            return;
        }

        std::cout << "\n[Parallel] Initializing TRUE domain decomposition:\n";
        ...
        std::cout << "[Parallel] Initialization complete\n\n";
    }
    ...
    void initialize_tls(const TwoLevelState& global_tls) {
        ...
        std::cout << "[Parallel] Initializing TLS for " << config.num_domains << " subdomains...\n";
        ...
        std::cout << "[Parallel] TLS initialization complete\n";
    }
